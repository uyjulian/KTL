...
static void regist_ktl_tjs_engine() {
	...
	if (global) {
		sprig::krkr::tjs::AddNewMember(global, SPRIG_KRKR_TJS_W("TJSEngine"), ktl::CreateNativeClassTJSEngine());
	}
}
static void unregist_ktl_tjs_engine() {
	...
	sprig::krkr::tjs::object_type global(::TVPGetScriptDispatch(), false);
	if (global) {
		sprig::krkr::tjs::DeleteMember(global, SPRIG_KRKR_TJS_W("TJSEngine"));
	}
}
