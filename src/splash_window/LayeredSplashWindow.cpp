...
static void regist_ktl_layered_window() {
	...
	if (global) {
		sprig::krkr::tjs::AddNewMember(global, SPRIG_KRKR_TJS_W("LayeredSplashWindow"), ktl::CreateNativeClassLayeredSplashWindow());
	}
}
static void unregist_ktl_layered_window() {
	...
	sprig::krkr::tjs::object_type global(::TVPGetScriptDispatch(), false);
	if (global) {
		sprig::krkr::tjs::DeleteMember(global, SPRIG_KRKR_TJS_W("LayeredSplashWindow"));
	}
}
