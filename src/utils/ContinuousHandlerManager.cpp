...
static void regist_ktl_continuous_handler_manager() {
	...
	if (global) {
		sprig::krkr::tjs::AddNewMember(global, SPRIG_KRKR_TJS_W("ContinuousHandlerManager"), ktl::CreateNativeClassContinuousHandlerManager());
	}
}
static void unregist_ktl_continuous_handler_manager() {
	...
	sprig::krkr::tjs::object_type global(::TVPGetScriptDispatch(), false);
	if (global) {
		sprig::krkr::tjs::DeleteMember(global, SPRIG_KRKR_TJS_W("ContinuousHandlerManager"));
	}
}
